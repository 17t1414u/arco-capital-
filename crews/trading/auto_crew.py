...
# ── 朝の Top10 状態保存（日中スキャンとの差分比較用） ─────────────────────────

MORNING_STATE_DIR = Path("outputs") / "daily-state"


def save_morning_top10(screening_results, analyzed_tickers: list[str]) -> None:
    """
    朝のスクリーニングTop10 + 深層分析済み銘柄を JSON で保存する。
    IntradayScanCrew が「新候補」を検出するために参照する。
    """
    MORNING_STATE_DIR.mkdir(parents=True, exist_ok=True)
    ...
    state = {
        ...
        "top10": [
            ...
            for r in screening_results[:10]
        ],
        "analyzed_tickers": analyzed_tickers,
    }
    path = MORNING_STATE_DIR / f"{today}_morning.json"
    path.write_text(json.dumps(state, ensure_ascii=False, indent=2), encoding="utf-8")
    print(f"💾 朝の状態を保存: {path}")


def load_morning_top10() -> dict:
    """今朝の Top10 状態を読み込む。なければ空 dict。"""
    today = date.today().isoformat()
    path = MORNING_STATE_DIR / f"{today}_morning.json"
    if not path.exists():
        return {}
...
class AutoCrew:
    ...
    def run(self) -> str:
        ...
        # ── STEP 0: 既存ポジション評価（エグジット判断） ─────────────
        review_report = ""
        if self.review_positions:
            print("📦 STEP 0/5: 既存ポジション評価＆エグジット判断...\n")
            try:
                review_crew = PositionReviewCrew(live=self.live)
                review_report = review_crew.run()
            except Exception as e:
                print(f"   ⚠️ ポジションレビュー失敗（継続します）: {e}")
                review_report = f"ポジションレビューエラー: {e}"

        # ── STEP 1: S&P500銘柄リスト取得 ─────────────────────────────
        print("\n📋 STEP 1/5: S&P500銘柄リストを取得中...\n")
        tickers = get_sp500_tickers()
        print(f"   → {len(tickers)}銘柄を取得\n")

        # ── STEP 2: バッチスクリーニング ──────────────────────────────
        print("🔍 STEP 2/5: バッチスクリーニング実行中...\n")
        ...
        print(f"\n🎯 STEP 3/5: Top{self.analyze_top}銘柄の深層分析開始...\n")
        ...
        for rank, candidate in enumerate(targets, 1):
            ...
            side = getattr(candidate, "side", "LONG")  # v2.3: LONG / SHORT
            print(f"\n--- {rank}/{self.analyze_top}: {ticker} [{side}] 分析中 ---\n")

            try:
                # v2.3: Path C (SHORT) 候補は context に明示して
                # FullTradingCrew に SHORT エントリー妥当性の評価を促す
                if side == "SHORT":
                    side_context = (
                        f"⚠️ Path C 過熱反転ショート候補: "
                        f"strategy.md v2.3 のショート建てルール (RSI>75, +10%乖離, "
                        f"MACD減速, 出来高萎縮) を満たした銘柄。"
                        f"SHORT エントリーの妥当性を評価し、推奨ならば "
                        f"intentional_short=True フラグ付きで SELL 発注すること。"
                    )
                else:
                    side_context = "通常の LONG 候補 (Path A 押し目買い or Path B 順張り)"
                ...
                # 承認 + (BUYシグナル または SHORT) で記録
                approved = "APPROVED" in result
                has_signal = ("BUY" in result and side == "LONG") or \
                             ("SELL" in result and side == "SHORT")
                if approved and has_signal:
                    executed_tickers.append(f"{ticker}({side})")
        ...
        try:
            save_morning_top10(
                screening_results,
                analyzed_tickers=[r.ticker for r in targets],
            )
        except Exception as e:
            print(f"   ⚠️ 朝の状態保存失敗（継続）: {e}")

        # ── STEP 4: SNS投稿生成 ───────────────────────────────────────
        sns_post = ""
        if self.generate_sns:
            print(f"\n📱 STEP 4/5: SNS投稿テキスト生成中...\n")
            try:
...
def _build_final_report(
    ...
    lines = [
        ...
        f"| 新規注文執行 | {len(executed_tickers)}銘柄 ({', '.join(executed_tickers) if executed_tickers else 'なし'}) |",
        f"",
    ]
    ...
    lines += [f"## スクリーニング結果 Top{min(5, len(screening_results))}"]
