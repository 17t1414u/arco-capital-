...
─── Xテーマ別スレッド（セクター・テーマ特集） ──────────────────
  # 事前定義テーマ（quantum/ai/ev/nuclear）
  python investment_main.py --mode x-theme --theme quantum

  # ティッカー手動指定（本番投稿）
  python investment_main.py --mode x-theme --tickers QBTS,IONQ,QUBT,RGTI --live
...
# Windows PowerShell の cp932 エンコードエラー対策
...
def run_auto(live: bool = False) -> None:
    """既存ポジション評価 → S&P500スクリーニング → 分析 → 執行の全自動パイプライン"""
    from crews.trading.auto_crew import AutoCrew

    print(f"\n🤖 全自動モード起動")
    print(f"   STEP 0: 既存ポジション評価 → エグジット判断")
    print(f"   STEP 1-3: S&P500 スキャン → Top10 → Top3深層分析 → 新規エントリー")
    print(f"   STEP 4-5: SNS投稿生成 → 日次レポート")
...
def run_position_review(live: bool = False) -> None:
    """既存ポジションのみ評価（エグジット判断のみ、新規エントリーなし）"""
    from crews.trading.position_review_crew import PositionReviewCrew

    mode_str = "⚠️ 実発注モード" if live else "ドライラン（判断のみ）"
    print(f"\n📦 ポジションレビューモード: {mode_str}")
    crew = PositionReviewCrew(live=live)
    result = crew.run()
    print("\n📋 レビュー結果:")
    print(result)


def run_intraday_scan(live: bool = False) -> None:
    """日中の軽量スキャン + 新候補のみ深層分析"""
    from crews.trading.intraday_scan_crew import IntradayScanCrew

    mode_str = "⚠️ 実発注モード" if live else "ドライラン（判断のみ）"
    print(f"\n📊 日中スキャンモード: {mode_str}")
    print(f"   軽量スクリーニング（Python のみ、LLMコスト $0）")
    print(f"   新候補が出た場合のみ LLM 分析（1銘柄 ~$0.3-0.5）")
    crew = IntradayScanCrew(live=live, max_new_analyses=2)
    result = crew.run()
    print("\n📋 スキャン結果:")
    print(result)
...
def run_x_thread(live: bool = False, ticker: str = "", topic: str = "") -> None:
    ...
    target = ticker or topic
    print(f"\n🐦 X投資スレッドクルー: {mode_str}")
    if target:
        print(f"   対象: {target}")

    crew = XInvestmentThreadCrew(dry_run=dry_run, ticker=target)
    result = crew.run()
    print("\n📋 実行結果:")
    print(result[:2000] + "..." if len(result) > 2000 else result)
...
def run_x_theme(
    ...
    """Xテーマ別スレッドを生成・投稿する（セクター・テーマ特集）"""
    from crews.trading.x_theme_crew import XThemeThreadCrew, THEME_MAP
    ...
    if tickers:
        ...
        if not resolved_theme:
            resolved_theme = "custom"
    elif theme and theme in THEME_MAP:
        ticker_list = THEME_MAP[theme]["tickers"]
    elif theme:
        print(f"❌ 不明なテーマ: {theme}")
        print(f"   使用可能テーマ: {', '.join(THEME_MAP.keys())}")
        print(f"   または --tickers QBTS,IONQ,QUBT,RGTI で直接指定")
        sys.exit(1)
    else:
        print("❌ --theme または --tickers を指定してください")
        print(f"   使用可能テーマ: {', '.join(THEME_MAP.keys())}")
        sys.exit(1)

    print(f"\n🎯 Xテーマスレッド: {resolved_theme.upper()} | {mode_str}")
    print(f"   対象銘柄: {', '.join(ticker_list)}")

    crew = XThemeThreadCrew(theme=resolved_theme, tickers=ticker_list, dry_run=dry_run)
    result = crew.run()
    print("\n📋 実行結果:")
    print(result[:2000] + "..." if len(result) > 2000 else result)
...
モード別使用例:
  ...
  python investment_main.py --mode sns --type monthly_summary --month "2026年4月"
    ...
    parser.add_argument(
        "--mode",
        required=True,
        choices=["auto", "review", "intraday-scan", "full", "trade", "analyse", "monitor", "optimize", "strategy-review", "sns", "x-thread", "x-theme"],
        help="実行モード",
    )
    parser.add_argument(
        "--live",
    ...
    parser.add_argument(
        "--theme",
        default="",
        help="テーマ名（x-theme モード用）例: quantum / ai / ev / nuclear",
    )
    parser.add_argument(
        "--tickers",
        default="",
        help="カンマ区切りティッカーリスト（x-theme モード用）例: QBTS,IONQ,QUBT,RGTI",
    )
    return parser
...
def main() -> None:
    ...
    if args.mode == "auto":
        ...
        run_position_review(live=args.live)

    elif args.mode == "intraday-scan":
        run_intraday_scan(live=args.live)
        ...
        run_x_thread(live=args.live, ticker=args.ticker, topic=args.topic)

    elif args.mode == "x-theme":
        run_x_theme(
            theme=args.theme,
